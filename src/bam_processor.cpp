...
void BamProcessor::read_and_filter_reads(BamCramMultiReader& reader, const std::string& chrom_seq, const RegionGroup& region_group,
	...
	while (reader.GetNextAlignment(alignment)){
		...
		if (alignment.Position() > region_group.start() || alignment.GetEndPosition() < region_group.stop()){
...
//					}
			...
			if (pass_one){
				...
				//auto aln_iter = potential_mates.find(aln_key); we don't add mates
				auto aln_iter = potential_mates.end();
			...
			std::string aln_key = file_label + trim_alignment_name(alignment);
			//auto aln_iter = potential_strs.find(aln_key); I don't think we should enter this
			auto aln_iter = potential_strs.end();
			if (aln_iter != potential_strs.end()){
				...
				//auto other_iter = potential_mates.find(aln_key);  no mate
				auto other_iter = potential_mates.end();
