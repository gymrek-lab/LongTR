...
//      //int num_stutter_artifacts = 100;
//      //std::cout << "num_stutter_artifacts = " << num_stutter_artifacts << std::endl;
...
//
//	for (int artifact_size = rep_info->max_deletion(); artifact_size <= rep_info->max_insertion(); artifact_size += period){ // Need to change here TODO
//	  int art_pos          = -1;
...
//}
void HapAligner::needleman_wunsch(const std::string& cent_seq, const std::string& read_seq, int& score) const {
...
void HapAligner::align_seq_to_hap(Haplotype* haplotype, bool reuse_alns,
  ...
  char first_hap_base = haplotype->get_first_char();
  int l;
  needleman_wunsch(seq_0, haplotype->get_seq(), l);
  left_prob = (double)l;
//  for (int j = 0; j < seq_len; ++j){
//    match_matrix[j]    = (seq_0[j] == first_hap_base ? base_log_correct[j] : base_log_wrong[j]) + left_prob;
//    insert_matrix[j]   = base_log_correct[j] + left_prob;
...
//    if (reuse_alns && block_index < haplotype->last_changed()){
//      haplotype_index += block_seq.size() + (block_index == 0 ? -1 : 0);
//      std::cout << "haplotype index increased in reuse " << haplotype_index << std::endl;
//      matrix_index     = seq_len*haplotype_index;
//
//    } TODO Update reuse, now it is not working
...
//          double match_emit             = (seq_0[j] == hap_char ? base_log_correct[j] : base_log_wrong[j]);
//
//          int homopolymer_len = 1; //TODO understand this
//
//          match_probs.push_back(insert_matrix[matrix_index-1]           + -0.6);
//          match_probs.push_back(match_matrix[matrix_index-seq_len-1]    + 0);
//          match_probs.push_back(deletion_matrix[matrix_index-seq_len-1] + -0.6);
//
//          match_matrix[matrix_index]    = match_emit + std::max(match_probs[0], std::max(match_probs[1], match_probs[2]));
//          insert_matrix[matrix_index]   = base_log_correct[j] + std::max(match_matrix[matrix_index-seq_len-1] + LOG_INS_TO_MATCH,
//                                        insert_matrix[matrix_index-1]  + LOG_INS_TO_INS);
...
//  }
//  delete [] L_log_probs;
//  left_prob = std::max(match_matrix[matrix_index], std::max(deletion_matrix[matrix_index], insert_matrix[matrix_index]));

  //std::cout << haplotype_index << " " << haplotype->cur_size() << std::endl;
//  assert(haplotype_index == haplotype->cur_size());
}
...
//  std::cout << "TOTAL LL " << total_LL << std::endl;
...
void HapAligner::process_reads(const std::vector<Alignment>& alignments, int init_read_index, const BaseQuality* base_quality, const std::vector<bool>& realign_read,
  ...
  for (unsigned int i = 0; i < alignments.size(); i++){
    ...
    seed_base = alignments[i].get_sequence().size() - 1;

    //std::cout << "Seed base " << seed_base << "alignment size" << alignments[i].get_sequence().size() << std::endl;
...
void HapAligner::process_read(const Alignment& aln, int seed_base, const BaseQuality* base_quality, bool retrace_aln,
  ...
  for (unsigned int j = 0; j < qual_string.size(); j++){
    ...
    base_log_wrong[j] = -0.2;
    base_log_correct[j] = -0.6;
}
  ...
  do {
    ...
    //std::cout << "alignment of sequence with size " << aln.get_sequence().size() << " to haplotype with size " << fw_haplotype_->get_seq().size()<< std::endl;
    ...
    std::cout << "alignment of sequence with size " << aln.get_sequence().size() << " to haplotype with size " << fw_haplotype_->get_seq().size() << " with l_prob " << l_prob << std::endl;
