...
void HapAligner::align_seq_to_hap_short(Haplotype* haplotype, bool reuse_alns,
...
//  double insert_matrix_[n][m];
  ...
  for (int j = 1; j < n; j++){
    for (int i = 1; i < m; i++){
	  ...
	  insertion_matrix[j * m + i] = MATCH + std::max(match_matrix[(j-1) * m + i] + coefficient*LOG_INS_TO_MATCH,
									insertion_matrix[(j-1) * m + i] + coefficient*LOG_INS_TO_INS);
      deletion_matrix[j * m + i] = std::max(match_matrix[j * m + i - 1]  + coefficient*LOG_DEL_TO_MATCH,
						   deletion_matrix[j * m + i - 1] + coefficient*LOG_DEL_TO_DEL);
    }
 }
//  for (int j = 0; j < n; j++){
//    for (int i = 0; i < m; i++){
//        std::cout << match_matrix[j * m + i] << " ";
//    }
//    std::cout << std::endl;
//  }
//  std::cout << std::endl;
// for (int j = 0; j < n; j++){
//    for (int i = 0; i < m; i++){
//        std::cout << insertion_matrix[j * m + i] << " ";
//    }
//    std::cout << std::endl;
//  }
//  std::cout << std::endl;
// for (int j = 0; j < n; j++){
//    for (int i = 0; i < m; i++){
//        std::cout << deletion_matrix[j * m + i] << " ";
//    }
//    std::cout << std::endl;
//  }
...
void HapAligner::process_read(const Alignment& aln, int seed_base, const BaseQuality* base_quality, bool retrace_aln,
			      double* prob_ptr, AlignmentTrace& trace, int short_){
   if (short_ == 0){
      //assert(seed_base != -1);
  ...
  do {
    ...
    double l_prob, r_prob;
    int max_index;
    align_seq_to_hap_short(fw_haplotype_, reuse_alns, base_seq, seed_base, base_log_wrong, base_log_correct,
		     l_match_matrix, l_insert_matrix, l_deletion_matrix, l_best_artifact_size, l_best_artifact_pos, l_prob);

    align_seq_to_hap_short(rev_haplotype_, reuse_alns, rev_rseq.c_str(), rev_rseq.size(), base_log_wrong+seed_base+1, base_log_correct+seed_base+1,
