...
| **snp-vcf**    phased_snps.vcf.gz     | Bgzipped input VCF file containing phased SNP genotypes for the samples to be genotyped. These SNPs will be used to physically phase TRs<br> **Why? You have available phased SNP genotypes**  
...
We've provided various *BED* files containing TR loci for different organisms, including humans, [here](https://github.com/HipSTR-Tool/HipSTR-references/)
...
#### INFO fields
INFO fields contains aggregated statistics about each genotyped TR in the VCF. The INFO fields reported by HipSTR primarily describe the TR's reference coordinates (START and END) and information about the allele counts (AC) and number of reads used to genotype all samples (DP).
